// GNSS/INS/LiDAR 组合导航 demo：把 GNSS/INS 与 LIO 接到同一条时间线。
// 滤波阶段 GNSS 位置/速度/航向与 LiDAR 点面 IEKF 都更新同一套 EKF。
// pose_meas（GNSS 锁位置、LiDAR 只改正姿态）见 demo/GNSS_LIO_INS。
#include <iomanip>
#include <iostream>
#include <memory>
#include <string>
...
// 追加顺序与 POST_MSF DataReader::BuildTimeline 一致：IMU → POS → VEL → HEADING → LIDAR。
enum Tag : int {
    TAG_IMU = 0,
    TAG_GNSS_POS = 1,
    TAG_GNSS_VEL = 2,
...
bool ParseArgs(int argc, char* argv[], std::string& config_file, std::string& output_dir,
    ...
    if (config_file.empty()) {
        std::cerr << "用法: gnss_lidar_ins --config=<yaml> [--output=<dir>] "
    ...
    for (size_t ev_idx = start_idx; ev_idx < events.size(); ++ev_idx) {
        ...
        switch (event.tag) {
            ...
            case TAG_LIDAR: {
                ...
                rec.quality_passed = true;
                const msf::NominalState before = s._nominal;
                updated = ekf.Update(factor, config._lidar._max_iterations + 1,
