...
class MotionDistortionNode : public rclcpp::Node
{
public:
  MotionDistortionNode() : Node("motion_distortion_node")
  {
    init();
  }
  ...
  bool initIO() {
    ...
    ins_sub_ = this->create_subscription<novatel_oem7_msgs::msg::INSPVA>(
      "/bynav/inspva", qos, std::bind(&MotionDistortionNode::ins_callback, this, std::placeholders::_1));

    lidar_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    "/iv_points", qos, std::bind(&MotionDistortionNode::ins_callback, this, std::placeholders::_1));
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<MotionDistortionNode>();
